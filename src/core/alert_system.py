...
def manual_add_weighted(src1, alpha, src2, beta, gamma, dst):
    dst[:] = np.clip(src1.astype(np.float64) * alpha + src2.astype(np.float64) * beta + gamma, 0, 255).astype(np.uint8)
    return dst


def manual_bgr_to_rgb(bgr):
    return np.ascontiguousarray(bgr[:, :, ::-1])


def manual_rgb_to_bgr(rgb):
...
class AlertSystem:
    ...
    def put_text_unicode(self, frame, text, position, color, font_size):
        frame_rgb = manual_bgr_to_rgb(frame)
        ...
        return manual_rgb_to_bgr(np.array(pil_image))
    ...
    def render_drowsiness_alert(self, frame, duration=None):
        ...
        manual_add_weighted(overlay, alpha, frame, 1 - alpha, 0, frame)
    ...
    def render_distraction_alert(self, frame):
        ...
        manual_add_weighted(overlay, 0.3, frame, 0.7, 0, frame)
    ...
    def render_fatigue_alert(self, frame):
        ...
        manual_add_weighted(overlay, 0.7, frame, 0.3, 0, frame)
